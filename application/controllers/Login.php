...
class Login extends CI_Controller {
    ...
    public function index()
    {
        if(isset($_POST['proses'])) {
            ...
            if ($this->form_validation->run() == FALSE)
            {
                ...
                if($this->Mod_login->check_db($username)->num_rows()==1) {
                    ...
                    //if($data) {
                        $userdata = array(
                            ...
                            'user_akses'  => $db->user_akses,
                        );

                    // print_r($userdata); die();
                    $this->session->set_userdata($userdata);

                    if($userdata['user_akses']=='1'){ //Administrator
                        $this->session->set_userdata('akses','Administrator');
                        redirect('dashboard');
                    
                    }else if($userdata['user_akses']=='2'){ //Anggota
                ...
                //Login Anggota
                else if($this->Mod_login->check_db2($username)->num_rows()==1) {

                    //cek verfied bycrpt menyamakan data yg di input dengan ada yg di database 
                    $db = $this->Mod_login->check_db2($username)->row();
    ...
    public function view_buku()
    {
        ...
        $config['base_url']   = base_url('login/view_buku/');
        $config['total_rows'] = $total;
        $config['per_page']   = 5;
        ...
        $data['buku']       = $this->Mod_buku->getDataBuku($limit, $offset);
