...
class Anggota extends MY_Controller {
    ...
    public function insert()
    {
        if(isset($_POST['save'])) {
            ...
            if($this->form_validation->run()==true){
                ...
                // Hapus validasi duplikasi sektor - sektor bisa sama untuk beberapa anggota
                ...
                if ($this->upload->do_upload('userfile')){
                    ...
                    $save  = array(
                        ...
                        'user_akses' => $this->input->post('akses')
                    );
        ...
        if(isset($_POST['update'])) {
            ...
            if(!empty($_FILES['userfile']['name'])) {
                ...
                if($this->form_validation->run()==true){
                    ...
                    $nis = $this->input->post('nis');
                    $nis_lama = $this->input->post('nis_lama'); // NIS sebelum diubah
                    
                    // Hapus validasi duplikasi sektor - sektor bisa sama untuk beberapa anggota
                    // $cek = $this->Mod_anggota->cekAnggotaUpdate($nis, $nis_lama);
                    // if($cek->num_rows() > 0){
                    //     $data['message'] = "<div class='alert alert-block alert-danger'>
                    //     <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                    //     <p><strong><i class='icon-ok'></i>Sektor</strong> Sudah Digunakan oleh Anggota Lain...!</p></div>"; 
                    //     $data['edit'] = $this->Mod_anggota->cekAnggota($nis_lama)->row_array();
                    //     $this->template->load('layoutbackend', 'anggota/anggota_edit', $data);
                    //     return;
                    // }
                    ...
                    if ($this->upload->do_upload('userfile')){
                        ...
                        $g = $this->Mod_anggota->getGambar($nis_lama)->row_array();
                        
                        //hapus gambar yg ada diserver
                        unlink('assets/img/anggota/'.$g['image']);

                        $this->Mod_anggota->updateAnggota($nis_lama, $save);
                        // echo "berhasil"; die();
    ...
    {
        $this->form_validation->set_rules('nis','Sektor','required|max_length[10]|alpha_numeric');
        $this->form_validation->set_rules('nama','Nama','required|max_length[50]');
        $this->form_validation->set_rules('jenis','Jenis Kelamin','required|max_length[2]');
        $this->form_validation->set_rules('tgl_lahir','Tanggal Lahir','required'); 
        $this->form_validation->set_rules('kelas','Kelas','required|max_length[500]');
        $this->form_validation->set_rules('username','Username','required|trim');
        $this->form_validation->set_rules('password','Password','required|trim');
        $this->form_validation->set_rules('akses','User Akses','required|max_length[2]');
